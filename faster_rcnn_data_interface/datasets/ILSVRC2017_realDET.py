...
class ilsvrc_det(imdb):
    def __init__(self, image_set, year, devkit_path='./data/ILSVRC2017_DET'):
        ...
        assert os.path.exists(self._devkit_path), \
          'ILSVRC path does not exist: {}'.format(self._devkit_path)
    ...
    def image_path_from_index(self, index):
        ...
        """
        if index.find('train') >= 0:
            image_path = os.path.join(self._data_path, 'Data','DET','train',
                                      index + self._image_ext)
        elif index.find('test') >= 0:
            image_path = os.path.join(self._data_path, 'Data','DET','test',
                                      index + self._image_ext)
        elif index.find('val') >= 0:
            image_path = os.path.join(self._data_path, 'Data','DET','val',
                                      index + self._image_ext)
        else:
    ...
    def _load_image_set_index(self):
        ...
        if self._image_set.find('train') >= 0 or self._image_set.find('test') >= 0 or self._image_set.find('val') >= 0:
    ...
    def _load_ilsvrc_annotation(self, index):
        ...
        """
        if index.find('train') >= 0:
            filename = os.path.join(self._data_path, 'Annotations', 'DET', 'train', index+'.xml')
            ...
            filename = os.path.join(self._data_path, 'Annotations', 'DET', 'val', index+'.xml')
            tree = ET.parse(filename)
            objs = tree.findall('object')
            num_objs = len(objs)
            boxes = np.zeros((num_objs, 4), dtype=np.uint16)
