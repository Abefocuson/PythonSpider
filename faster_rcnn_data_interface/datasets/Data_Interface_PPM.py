...
class ilsvrc(imdb):
    ...
    def _load_image_set_index(self):
        ...
        if self._image_set.find('train') >= 0:
            ...
            return image_index
        elif self._image_set.find('test') == 0:
            image_set_file = os.path.join(self._data_path, 'ImageSets', 'VID',
                                          self._image_set + '.txt')
            assert os.path.exists(image_set_file), \
                'Path does not exist: {}'.format(image_set_file)
            ...
            return image_index
        elif self._image_set.find('val') >= 0:
            image_set_file = os.path.join(self._data_path, 'ImageSets', 'VID',
                                          self._image_set + '.txt')
            assert os.path.exists(image_set_file), \
                'Path does not exist: {}'.format(image_set_file)
    ...
    def _load_ilsvrc_annotation(self, index):
        ...
        if index.find('train') >= 0:
            ...
            filename = os.path.join(self._data_path, 'Annotations', 'VID', 'val', index[0:30]+'.xml')
            tree = ET.parse(filename)
            objs = tree.findall('object')
            num_objs = len(objs)
            boxes = np.zeros((num_objs, 4), dtype=np.uint16)
            ...
            for ix, obj in enumerate(objs):
                ...
                x2 = float(bbox.find('xmax').text)
                y2 = float(bbox.find('ymax').text)
                print ('aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa:', bbox)
                if x1<1 or y1<1:
                    raise KeyError('')
                cls = self._class_to_ind[obj.find('name').text.lower().strip()]
                boxes[ix, :] = [x1, y1, x2, y2]
